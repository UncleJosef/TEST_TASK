...
BEGIN
	...
	INSERT INTO [PLAYER].[T_STATE](LAST_TRACK_ID, MAX_TRACK_ID, MIN_TRACK_ID, BARRIER, TRACK_10_1, TRACK_10_2, FLAG)
		SELECT
			MIN(ID),
			MAX(ID), 
			MIN(ID), 
			@BARRIER,
			(SELECT ID FROM [PLAYER].T_TRACKS WHERE NUMBER=3),
			(SELECT ID FROM [PLAYER].T_TRACKS WHERE NUMBER=4),
			0
	...
	--SELECT [TIME], 0, 
	--	CAST(ROW_NUMBER() OVER(ORDER BY TRACK_ID) AS FLOAT)*(CASE WHEN (TIME=5) THEN 0.0555555*2 ELSE 0.0555555 END)
	--FROM [PLAYER].T_TRACK_COUNTER
